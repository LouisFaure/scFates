...
def cellrank_to_tree(
    ...
    **kwargs
    ...
    if n_states == 2:
        adata.obsm["X_fates"]=np.vstack([np.array(adata.obsm['to_terminal_states'][:,0].flatten()),
                                         adata.obs[time]]).T
        logg.hint(
        ...
        logg.hint(
            "with .obsm['X_fates'], created by combining:\n"
            "    .obsm['X_fate_simplex_fwd'] (from cr.pl.circular_projection) and adata.obs['"+time+"']\n")
        cr.pl.circular_projection(adata,keys=["kl_divergence"])
        plt.close()

        adata.obsm["X_fates"]=np.concatenate([adata.obsm["X_fate_simplex_fwd"],
                                              adata.obs[time].values.reshape(-1,1)],axis=1)
